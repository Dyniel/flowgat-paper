#!/bin/bash
#SBATCH --job-name=paper_train_subend
...
# ---------------------------------------------------------------------------
# 24_train_subend.sh — Phase E7 step 2: sUbend training.
#
# 4 variants × 3 seeds = 12 tasks. Same recipe as Cosserat sweep (job 19),
# pointed at the sUbend NPZ datasets built by job 23.
#
# Array idx → (variant, seed):
#   0-2:  subend_withleak       {1337, 2026, 777}
#   3-5:  subend_leak_dir_only  {1337, 2026, 777}
#   6-8:  subend_leak_mag_only  {1337, 2026, 777}
#   9-11: subend_noleak         {1337, 2026, 777}
#
# Submit:  sbatch jobs/24_train_subend.sh
# Walltime per run: ~4-8h (50k nodes, 25 frames × 15 cases = 375 samples).
# ---------------------------------------------------------------------------
...
VARIANTS=(
  "subend_withleak"      "subend_withleak"      "subend_withleak"
  "subend_leak_dir_only" "subend_leak_dir_only" "subend_leak_dir_only"
  "subend_leak_mag_only" "subend_leak_mag_only" "subend_leak_mag_only"
  "subend_noleak"        "subend_noleak"        "subend_noleak"
)
...
echo "==========================================================================="
echo " SUBEND TRAIN | variant=${VARIANT} seed=${SEED} | $(date)"
echo "==========================================================================="
...
if [[ ! -f "$BEST_CKPT" ]]; then
  echo "[24] ERROR: missing $BEST_CKPT" >&2
  exit 1
fi
...
"$PYBIN" "$SRC_DIR/dump_predictions.py" \
  ...
  echo "[24] WARN: dump_predictions failed"

echo "[24] DONE | variant=${VARIANT} seed=${SEED}"
