# -*- coding: utf-8 -*-
"""
Build leakage-variant Womersley NPZ datasets from data/npz_womersley.

Reads the noleak Womersley (already global PCA tangent + unit local radius),
and produces:
  npz_womersley_withleak       : x[:,3:6] = unit(y),  x[:,8] = u_mean/u_char
  npz_womersley_leak_dir_only  : x[:,3:6] = unit(y),  x[:,8] = noleak local_R
  npz_womersley_leak_mag_only  : x[:,3:6] = noleak PCA, x[:,8] = u_mean/u_char
"""
...
def main() -> None:
    ...
    ap.add_argument("--src_dir", default="data/npz_womersley")
    ...
    targets = {
        "wl": out_root / "npz_womersley_withleak",
        "do": out_root / "npz_womersley_leak_dir_only",
        "mo": out_root / "npz_womersley_leak_mag_only",
    }
    ...
    print(f"[wm-variants] processing {len(cases)} cases")
    ...
    for i, case in enumerate(cases):
        ...
        a_wl["feature_mode"] = np.array("womersley_withleak_v1")
        ...
        a_do["feature_mode"] = np.array("womersley_leak_dir_only_v1")
        ...
        a_mo["feature_mode"] = np.array("womersley_leak_mag_only_v1")
        np.savez_compressed(targets["mo"] / f"{case}.npz", **a_mo)

        if i % 5 == 0 or i == len(cases) - 1:
            print(f"  [{i+1}/{len(cases)}] {case}")

    print(f"[wm-variants] DONE; wrote to: {', '.join(str(p) for p in targets.values())}")
